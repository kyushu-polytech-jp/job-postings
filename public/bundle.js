...
var init_common_341bda9b_esm = __esm({
  "node_modules/@firebase/firestore/dist/common-341bda9b.esm.js"() {
    ...
    L = "The current tab is not in the required state to perform this operation. It might be necessary to refresh the browser tab.";
    ...
    Le = "documentOverlays";
    We = "globals";
    Ge = [...[...[...[...[z, H, ee, W, ue, Q, Re, he], de], _e], Ae], fe, pe];
    ze = [...Ge, Le];
    je = [z, H, ee, te, ue, Q, Re, he, de, _e, Ae, fe, pe, Le];
    He = je;
    Je = [...He, we, Ce, xe];
    Xe = [...Je, We];
    ...
    tt = "server_timestamp";
    nt = "__type__";
    rt = "__previous_value__";
    it = "__local_write_time__";
    DatabaseInfo = class {
      /**
    ...
    It = new SortedMap(DocumentKey.comparator);
    Et = new SortedSet(DocumentKey.comparator);
    Rt = new SortedSet(__PRIVATE_primitiveComparator);
    ...
    dt = null;
    mt = new Integer([4294967295, 4294967295], 0);
...
This typically indicates that your device does not have a healthy Internet connection at the moment. The client will operate in offline mode until it is able to successfully connect to the backend.`;
    ...
    FirestoreSettingsImpl = class {
      constructor(e) {
        ...
        } else this.host = e.host, this.ssl = (_a = e.ssl) != null ? _a : rn;
...
var Ut, Ht2, DocumentSnapshot$1, QueryDocumentSnapshot$1, AppliableConstraint, QueryConstraint, QueryFieldFilterConstraint, QueryCompositeFilterConstraint, QueryOrderByConstraint, QueryLimitConstraint, QueryStartAtConstraint, SnapshotMetadata, DocumentSnapshot, QueryDocumentSnapshot, QuerySnapshot;
var init_index_esm6 = __esm({
  "node_modules/@firebase/firestore/dist/index.esm.js"() {
    init_index_esm4();
    init_index_esm2();
    init_common_341bda9b_esm();
    init_common_341bda9b_esm();
    init_index_esm();
    init_bloom_blob_es2018();
    init_index_esm3();
...
var require_main = __commonJS({
  "public/main.ts"() {
    ...
    var ownerNameKanaInput = document.getElementById("ownerNameKanaInput");
    ...
    var pdfDisplayArea = document.getElementById("pdfDisplayArea");
    var prevPageButton = document.getElementById("prevPageButton");
    var nextPageButton = document.getElementById("nextPageButton");
    var pageInfoSpan = document.getElementById("pageInfo");
    var PAGE_SIZE = 10;
    var SERVER_BATCH_SIZE = 50;
    var currentBaseQuery = null;
    var scanStartMarkers = [];
    var currentPageIndex = 0;
    var reachedEnd = false;
    var totalCount = 0;
    var totalCountKnown = false;
    ...
    async function searchJobPostings(direction = "initial") {
      var _a, _b;
      ...
      if (direction === "initial") {
        ...
        if (departmentSelect.value) {
          constraints.push(
            where(
              ...
              "array-contains",
              isNaN(Number(departmentSelect.value)) ? departmentSelect.value : Number(departmentSelect.value)
            )
          );
        }
        ...
        let base = collection(db, "jobPostings");
        if (constraints.length) base = query(base, ...constraints);
        currentBaseQuery = hasPrefRange ? query(base, orderBy("prefecture", "asc")) : query(base, orderBy("__name__", "asc"));
        scanStartMarkers = [null];
        currentPageIndex = 0;
        reachedEnd = false;
        ...
        (async () => {
          try {
            ...
            let last = null;
            while (true) {
              const batchQuery = last ? query(currentBaseQuery, startAfter(last), limit(SERVER_BATCH_SIZE)) : query(currentBaseQuery, limit(SERVER_BATCH_SIZE));
              const snapshot = await getDocs(batchQuery);
              if (snapshot.empty) break;
              for (const d of snapshot.docs) {
                const kana = normalizeKana(d.data().ownerNameKana || "");
      ...
      let start = null;
      if (direction === "next") start = (_a = scanStartMarkers[currentPageIndex]) != null ? _a : null;
      if (direction === "prev") start = (_b = scanStartMarkers[Math.max(0, currentPageIndex - 2)]) != null ? _b : null;
      reachedEnd = false;
      const results = [];
      let lastScanned = start;
      while (results.length < PAGE_SIZE) {
        const pageBatchQuery = lastScanned ? query(currentBaseQuery, startAfter(lastScanned), limit(SERVER_BATCH_SIZE)) : query(currentBaseQuery, limit(SERVER_BATCH_SIZE));
        ...
        lastScanned = lastInspected != null ? lastInspected : pageSnapshot.docs[pageSnapshot.docs.length - 1];
        if (results.length >= PAGE_SIZE) break;
      }
      if (results.length === 0) {
      ...
      } else if (scanStartMarkers[currentPageIndex] === void 0) {
        scanStartMarkers[currentPageIndex] = start != null ? start : null;
      }
      prevPageButton.disabled = currentPageIndex <= 1;
      const hasNextStart = scanStartMarkers[currentPageIndex] !== void 0 && scanStartMarkers[currentPageIndex] !== null;
      nextPageButton.disabled = reachedEnd && results.length < PAGE_SIZE || !hasNextStart;
      pageInfoSpan.textContent = totalCountKnown ? `\u30DA\u30FC\u30B8 ${currentPageIndex} / ${Math.max(1, Math.ceil(totalCount / PAGE_SIZE))}` : `\u30DA\u30FC\u30B8 ${currentPageIndex}`;
    }
    async function displayPdf(year, reception) {
      let numericReception = null;
