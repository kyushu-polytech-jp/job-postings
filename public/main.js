...
const ownerNameKanaInput = document.getElementById('ownerNameKanaInput');
...
let currentBaseQuery = null;
let scanStartMarkers = [];
...
function toKatakana(src) {
    return src.replace(/[\u3041-\u3096]/g, c => String.fromCharCode(c.charCodeAt(0) + 0x60));
}
...
async function loadDepartments() {
    try {
        const deptSnap = await getDocs(query(collection(db, 'departments'), where('isValid', '==', true)));
        const items = [];
...
async function searchJobPostings(direction = 'initial') {
    ...
    if (direction === 'initial') {
        ...
        if (departmentSelect.value) {
            constraints.push(where('recruitingDepartments', 'array-contains', isNaN(Number(departmentSelect.value))
        ...
        (async () => {
            try {
                ...
                let last = null;
                while (true) {
                    const batchQuery = last
                        ? query(currentBaseQuery, startAfter(last), limit(SERVER_BATCH_SIZE))
                        : query(currentBaseQuery, limit(SERVER_BATCH_SIZE));
    ...
    let start = null;
    if (direction === 'next')
        start = (_a = scanStartMarkers[currentPageIndex]) !== null && _a !== void 0 ? _a : null;
    if (direction === 'prev')
        start = (_b = scanStartMarkers[Math.max(0, currentPageIndex - 2)]) !== null && _b !== void 0 ? _b : null;
    // ★毎回リセット（末尾残留フラグを防止）
    reachedEnd = false;
    const results = [];
    let lastScanned = start;
    while (results.length < PAGE_SIZE) {
        const pageBatchQuery = lastScanned
            ? query(currentBaseQuery, startAfter(lastScanned), limit(SERVER_BATCH_SIZE))
            : query(currentBaseQuery, limit(SERVER_BATCH_SIZE));
        ...
        for (const d of pageSnapshot.docs) {
            ...
            if (kana.includes(kanaNeedle)) {
                ...
                if (results.length >= PAGE_SIZE)
                    break; // ページ分たまったらバッチ途中で抜ける
            }
        }
        // ★  次ページの開始点は「最後に検査した位置」から
        lastScanned = lastInspected !== null && lastInspected !== void 0 ? lastInspected : pageSnapshot.docs[pageSnapshot.docs.length - 1];
    ...
    if (lastScanned) {
        ...
        scanStartMarkers[currentPageIndex] = start !== null && start !== void 0 ? start : null;
    }
    // --- ページングボタン ---
    prevPageButton.disabled = currentPageIndex <= 1;
    const hasNextStart = scanStartMarkers[currentPageIndex] !== undefined &&
